...
app.post("/postRSA", (req: Request, res: Response) => {
  try {
    ...
    let n = req.body.n;
    console.log(
      `Valor de generadores RSA PUBLICA CLIENTE --> e: ${base64ToBigint(e)}\nn: ${base64ToBigint(n)}`,
    );
...
app.post(
  "/encrypt",
  (req: Request<{}, CipherMsg, { message: string }, RequestMsg, {}>, res) => {
    if (pubKeyClient && req.body.message) {
      const messageToEncrypt =
        "El servidor ha cifrado con tu publica el siguiente mensaje:" +
        req.body.message;
      const encryptedMessage = pubKeyClient.encrypt(
        textToBigint(messageToEncrypt),
      );
      ...
      res
        .status(500)
        .json({ error: "RSA key pair not available or message missing" });
    }
  },
);
...
app.post("/decrypt", (req: Request, res: Response) => {
  if (keyPair && req.body) {
    ...
    res
      .status(500)
      .json({
        error: "RSA key pair not available or encrypted message missing",
      });
  }
});
